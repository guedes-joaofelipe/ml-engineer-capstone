...
class SVD():
    ...
    def __init__(self, learning_rate=.005, regularization=0.02, n_epochs=20,
                 n_factors=100, min_rating=1, max_rating=5, verbose=True):
    ...
    def predict(self, X, clip=True):
        ...
        for u_id, i_id in zip(X['u_id'], X['i_id']):
            predictions.append(self.predict_pair(u_id, i_id, clip))
    ...
    def _on_epoch_begin(self, epoch_ix):
        ...
        print('Epoch {}/{}'.format(epoch_ix + 1, self.n_epochs), end=end) if self.verbose else ''
    ...
    def _on_epoch_end(self, start, val_loss=None, val_rmse=None, val_mae=None):
        ...
        if self.early_stopping:
            print('val_loss: {:.2f}'.format(val_loss), end=' - ') if self.verbose else ''
            print('val_rmse: {:.2f}'.format(val_rmse), end=' - ') if self.verbose else ''
            print('val_mae: {:.2f}'.format(val_mae), end=' - ') if self.verbose else ''

        print('took {:.1f} sec'.format(end - start)) if self.verbose else ''
